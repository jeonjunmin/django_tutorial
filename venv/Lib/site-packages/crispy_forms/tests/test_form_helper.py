...
def test_media_is_included_by_default_with_uniform():
    ...
    html = render_crispy_form(form)
    assert "python_basic.css" in html
    assert "python_basic.js" in html
...
def test_media_removed_when_include_media_is_false_with_uniform():
    ...
    html = render_crispy_form(form)
    assert "python_basic.css" not in html
    assert "python_basic.js" not in html
...
def test_template_context():
    helper = FormHelper()
    helper.attrs = {
        "id": "python_basic-form",
        "class": "python_basic-forms",
        "action": "submit/python_basic/form",
        "autocomplete": "off",
    }
    node = CrispyFormNode("form", "helper")
    context = node.get_response_dict(helper, {}, False)

    assert context["form_id"] == "python_basic-form"
    assert context["form_attrs"]["id"] == "python_basic-form"
    assert "python_basic-forms" in context["form_class"]
    assert "python_basic-forms" in context["form_attrs"]["class"]
    assert context["form_action"] == "submit/python_basic/form"
    assert context["form_attrs"]["action"] == "submit/python_basic/form"
    assert context["form_attrs"]["autocomplete"] == "off"


def test_template_context_using_form_attrs():
    helper = FormHelper()
    helper.form_id = "python_basic-form"
    helper.form_class = "python_basic-forms"
    helper.form_action = "submit/python_basic/form"
    node = CrispyFormNode("form", "helper")
    context = node.get_response_dict(helper, {}, False)
...
def test_template_helper_access():
    ...
    assert helper["form_id"] == "python_basic-form"
